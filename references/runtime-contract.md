...
Default base URL: https://api.v2fun.art/api/v1. Credentials are sent only to the configured API; signed asset downloads do not include Authorization. Doctor only checks local readiness and never validates account authentication. Balance performs a read-only remote query. Neither command generates assets.
